...
//   4) Insert a row into demon_spin_history.
//   5) Return JSON: { success, prize_index, prize_value, remaining_spins }
//
...
if ($spinAllowance <= 0) {
    ...
    if ($lastSpin) {
        ...
        $diff = time() - $lastTime;
        if ($diff < 24 * 3600) {
            $remainingSecs = 24 * 3600 - $diff;
            $h = floor($remainingSecs / 3600);
            $m = floor(($remainingSecs % 3600) / 60);
            echo json_encode([
                'success'         => false,
...
// Build a zero‐indexed PHP array of just (id, label, value) for JS
$wheelData = [];
...
$chosenKey   = $wheelData[$chosenIndex]['id'];    // using segment ID as prize_key in history

// 3) Begin transaction: insert a log + update user balance + possibly decrement spin_allowance + insert spin_history
try {
    $pdo->beginTransaction();

    // a) Insert into demon_credit_logs as "Wheel Spin"
    $desc = $chosenValue > 0
    ...
    if ($spinAllowance > 0) {
        ...
        $spinAllowance--; // reflect the new amount for response
    }

    // d) Insert into demon_spin_history (quest_id left as NULL for now)
    $histStmt = $pdo->prepare("
        INSERT INTO demon_spin_history
          (user_id, quest_id, prize_key, reward_amount)
        VALUES
          (:uid, :quest, :pkey, :amt)
    ");
    $histStmt->execute([
        'uid'   => $userId,
        'quest' => null,          // no quest tied yet
        'pkey'  => $chosenKey,    // using segment’s ID as prize_key
        'amt'   => $chosenValue
    ]);
